...
# compress
#
mencoder -ovc lavc -lavcopts vcodec=$codec:vpass=1:$opt -mf type=$fmt:w=$width:h=$height:fps=25 -nosound -o /dev/null mf://\*.$fmt
mencoder -ovc lavc -lavcopts vcodec=$codec:vpass=2:$opt -mf type=$fmt:w=$width:h=$height:fps=25 -nosound -o output.avi mf://\*.$fmt
